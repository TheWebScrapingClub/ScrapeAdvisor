...
class TestSpider(scrapy.Spider):
    ...
    def start_requests(self):
        """Generate requests with Decodo proxy configuration"""
        proxy = os.getenv("DECODO_UNBLOCKER", "")
        
        for url in self.start_urls:
            website = self._identify_website(url)
            antibot_type = self._identify_antibot_type(url)
            
            # Prepare headers - only add custom header for specific websites
            headers = {}
            if website in ['loccitane.com', 'canadagoose.com', 'realestate.com.au']:
