...
class TestSpider(scrapy.Spider):
    ...
    def start_requests(self):
        """Generate requests with ScraperAPI proxy configuration based on website"""
        proxy = os.getenv("SCRAPERAPI_KEY", "")
        proxy_js = os.getenv("SCRAPERAPI_JS_KEY", "")
        
        for url in self.start_urls:
            website = self._identify_website(url)
            antibot_type = self._identify_antibot_type(url)
            
            # Determine proxy configuration based on website
            proxy_url = self._get_proxy_for_website(website, proxy, proxy_js)
            print(proxy_url)
    ...
    def _get_proxy_for_website(self, website, proxy, proxy_js):
        """Determine the appropriate proxy configuration for each website"""
        # Websites that require JavaScript rendering
        js_websites = ['loccitane.com', 'canadagoose.com', 'realestate.com.au','leboncoin.fr']
        
        if website in js_websites:
            # Use JavaScript proxy for sites that need JS rendering
            return proxy_js
        else:
            # Use standard proxy for other sites
            return proxy
